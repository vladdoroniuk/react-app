...
export default class AuthService {
  ...
  removeUserFromConversation(
    ...
    return instance.get(
      API_URL + "user/remove/" + userIdToRemove + "/group/" + groupUrl
    );
  }

  removeAdminUserInConversation(
    userIdToRemove: string | number,
    groupUrl: string
  ) {
    return instance.get(
      API_URL + "user/remove/admin/" + userIdToRemove + "/group/" + groupUrl
    );
  }

  grantUserAdminInConversation(
    userIdToGrant: number | string,
    groupId: string
  ) {
    return instance.get(
      API_URL + "user/grant/" + userIdToGrant + "/group/" + groupId
    );
  }
