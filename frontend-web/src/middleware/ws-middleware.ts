...
function initWsAndSubscribe(
  ...
  if (wsClient) {
    wsClient.onConnect = () => {
      ...
      mainSubscribe = wsClient.subscribe(
        `/topic/user/${userId}`,
        (res: IMessage) => {
          ...
          switch (data.action) {
            ...
            case TransportActionEnum.ADD_CHAT_HISTORY:
              ...
              const currentMessages: FullMessageModel[] =
                store.getState().WebSocketReducer.chatHistory;
              store.dispatch(
                setGroupMessages(newMessages.concat(currentMessages))
              );
              break;
      ...
      publishWs(
        ...
        new TransportModel(
          userId || 0,
          TransportActionEnum.INIT_USER_DATA,
          wsUserTokenValue
        )
      );
    };
    ...
    switch (action.type) {
      ...
      case FETCH_GROUP_MESSAGES:
        ...
        publishWs(
          ...
          new TransportModel(
            model.userId || 0,
            TransportActionEnum.FETCH_GROUP_MESSAGES,
            undefined,
            model.groupUrl,
            undefined,
            model.messageId
      ...
      case MARK_MESSAGE_AS_SEEN:
        ...
        publishWs(
          ...
          new TransportModel(
            model.userId || 0,
            TransportActionEnum.MARK_MESSAGE_AS_SEEN,
            undefined,
            model.groupUrl
          )
        );
      ...
      case GRANT_USER_ADMIN:
        publishWs(
          ...
          new TransportModel(
            model.userId || 0,
            TransportActionEnum.GRANT_USER_ADMIN,
            model.userToken,
            model.groupUrl
          )
        );
 ...
 * @param userId
 */
function updateGroupsWithLastMessageSent(
  store: Store,
  value: FullMessageModel,
  userId: number
) {
